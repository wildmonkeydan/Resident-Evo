    ...
    public class WheelVehicle : MonoBehaviour {
        ...
        void Update()
        {
            foreach (ParticleSystem gasParticle in gasParticles)
            {
                ...
                em.rateOverTime = handbrake ? 0 : Mathf.Lerp(em.rateOverTime.constant, Mathf.Clamp(150.0f * throttle, 5.0f, 40.0f), 0.1f);
            }
            ...
            if (gamepad != null)
            {
                ...
                switch (input)
                {
                    ...
                    case "Brake":
                        ...
                        break;
                    case "Steering":
                        axis = gamepad.leftStick.ReadValue().x;
                        break;
                    case "Jump":
                        axis = gamepad.crossButton.ReadValue();
                        break;
                    case "Drift":
                        axis = gamepad.squareButton.ReadValue();
                        break;
                }
